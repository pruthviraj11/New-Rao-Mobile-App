                                ...
                                @foreach ($application_statuses as $application_status)
                                    <div class="row mb-3">
                                        ...
                                        <div class="col-md-3">
                                            ...
                                            <select name="status_value" id="status_value_{{ $application_status->id }}"
                                                class="form-select">
                                                @foreach ($status_value as $status)
                                                    <option value="{{ $status }}"
                                                        @if ($application_status->status_value == $status) selected @endif>
                                                        {{ $status }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="col-md-3">
                                            ...
                                            <input type="number" id="status_order_{{ $application_status->id }}"
                                                class="form-control"
                                                value="{{ $application_status->status_order ?? $application_status->order }}">
                                        </div>
                                        <div class="col-md-4">
                                            <label for="status_date_{{ $application_status->id }}">Date:</label>
                                            <input type="date" id="status_date_{{ $application_status->id }}"
                                                name="date" class="form-control"
                                                value="{{ $application_status->status_date ?? '' }}">
                                            <button class="btn btn-primary mt-2 submitStep"
                                                data-user-id="{{ $user->id }}"
                                                data-application-status-id="{{ $application_status->id }}">Save</button>
                                        </div>
                                    </div>
                                @endforeach
